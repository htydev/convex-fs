...
export const gcExpiredUploads = internalAction({
    ...
    handler: async (ctx) => {
        ...
        const results = await Promise.all(expired.map(async (upload) => {
            try {
                const result = await store.delete(upload.blobId);
                if (result.status === "not_found") {
                    console.log(`[UGC] Blob ${upload.blobId} not found in storage (upload abandoned)`);
                }
                return { status: result.status, upload };
            }
            catch (error) {
                console.log(`[UGC] Failed to delete blob ${upload.blobId} from storage: ${error}`);
        ...
        console.log(`[UGC] Deleted ${deletedCount} from storage, ${notFoundCount} not found, ${errorCount} errors`);
        ...
        if (okToDelete.length > 0) {
            ...
            console.log(`[UGC] Removed ${okToDelete.length} upload records from database`);
        }
...
export const findOrphanedBlobs = internalQuery({
    ...
    handler: async (ctx, args) => {
        const blobs = await ctx.db
            .query("blobs")
            .withIndex("refCountUpdatedAt", (q) => q.eq("refCount", 0).lt("updatedAt", args.threshold))
            .take(args.limit);
        return blobs.map((b) => ({
            _id: b._id,
            blobId: b.blobId,
...
export const gcOrphanedBlobs = internalAction({
    ...
    handler: async (ctx) => {
        ...
        const store = createBlobStore(config.storage);
        const results = await Promise.all(orphaned.map(async (blob) => {
            try {
                const result = await store.delete(blob.blobId);
                if (result.status === "not_found") {
                    // Blob already gone from storage - still clean up DB record
                    console.log(`[BGC] Blob ${blob.blobId} already deleted from storage`);
                }
                return { status: result.status, blob };
            }
            catch (error) {
                console.log(`[BGC] Failed to delete blob ${blob.blobId} from storage: ${error}`);
                return { status: "error", blob };
            }
        }));
        // Categorize results
        const okToDelete = results
        ...
        const notFoundCount = results.filter((r) => r.status === "not_found").length;
        const errorCount = results.filter((r) => r.status === "error").length;
        console.log(`[BGC] Deleted ${deletedCount} from storage, ${notFoundCount} already gone, ${errorCount} errors`);
        ...
        if (okToDelete.length > 0) {
            ...
            console.log(`[BGC] Removed ${okToDelete.length} blob records from database`);
        }
